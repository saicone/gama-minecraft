package com.saicone.gama.minecraft.util;
...
public class MavenMirror {
    ...
    @NotNull
    @SuppressWarnings("deprecation")
    public static String get(int timeout) throws IOException {
        if (cached != null) {
            return cached;
        }
    ...
    @NotNull
    public static CompletableFuture<String> getAsync(@NotNull Executor executor) {
        if (cached != null) {
            return CompletableFuture.completedFuture(cached);
        }
        return CompletableFuture.supplyAsync(() -> getOrDefault(DEFAULT), executor);
    }
}
